...
namespace FMOD
{
    public class Channel
    {
        ...
        public RESULT getDSPClock(ref ulong dspclock, ref ulong parentclock)
        {
            return FMOD_Channel_GetDSPClock(channelraw, ref dspclock, ref parentclock);
        }

        public RESULT setDelay(ulong dspclock_start, ulong dspclock_end, bool stopchannels = true)
        {
            return FMOD_Channel_SetDelay(channelraw, dspclock_start, dspclock_end, stopchannels ? 1 : 0);
        }

        public RESULT addFadePoint(ulong dspclock, float volume)
        {
            return FMOD_Channel_AddFadePoint(channelraw, dspclock, volume);
        }
        ...
        private static extern RESULT
            FMOD_Channel_GetDSPClock(IntPtr channel, ref ulong dspclock, ref ulong parentclock);

        [DllImport(VERSION.dll)]
        private static extern RESULT
            FMOD_Channel_SetDelay(IntPtr channel, ulong dspclock_start, ulong dspclock_end, int stopchannels);
        
        [DllImport(VERSION.dll)]
        private static extern RESULT
            FMOD_Channel_AddFadePoint(IntPtr channel, ulong dspclock, float volume);
